# Awards Strategy

This file maps projects to the hackathon tracks, sponsor awards, and merit
badges we are targeting.

## Main Tracks

| Track | Fit |
| --- | --- |
| Backyard AI | Useful everyday apps with clear real-world value |
| Thousand Token Wood | Whimsical, playful, creative apps |
| Community Choice | Clear demo, broad appeal, strong presentation |

## Sponsor Targets

| Sponsor | Best Project Fit |
| --- | --- |
| OpenBMB | InnerSpace, Pocket Tutor |
| OpenAI | InnerSpace, Roast My Repo |
| Black Forest Labs | Flux Costume Booth |
| Cohere | Tiny Quest Radio |
| JetBrains | Roast My Repo |
| Modal | Flux Costume Booth, Modal-Tuner |

## Merit Badges

| Badge | Practical Target |
| --- | --- |
| Off the Grid | Local runtime or GGUF apps without third-party inference APIs |
| Well-Tuned | Fine-tuned adapter published on Hugging Face |
| Off-Brand | Custom UI beyond default Gradio styling |
| Llama Champion | GGUF model running through llama.cpp |
| Sharing is Caring | Public dataset, traces, or training outputs on Hugging Face |
| Field Notes | Build report with lessons, architecture, and results |

## Project Award Map

| Project | Track | Sponsors | Badge Targets |
| --- | --- | --- | --- |
| InnerSpace | Backyard AI | OpenBMB, OpenAI | Well-Tuned, Off-Brand, Tiny Titan |
| Pocket Tutor | Backyard AI | OpenBMB | Off-Brand, Best Agent, Well-Tuned |
| Flux Costume Booth | Thousand Token Wood | BFL, Modal | Well-Tuned, Off-Brand, Best Demo |
| Tiny Quest Radio | Thousand Token Wood | Cohere | Llama Champion, Off the Grid, Tiny Titan |
| Roast My Repo | Backyard AI | JetBrains, OpenAI | Best Agent, Sharing is Caring, Off-Brand |
| Modal-Tuner | Backyard AI | Modal | Bonus Quest Champion, Field Notes |
